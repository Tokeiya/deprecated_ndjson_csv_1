...
mod test {
	...
	fn array() {
		let mut parser = super::array();
		let (act, rem) = parser.parse(r#"[1, 1.2,  true,       false,
		"hello",null]"#).unwrap();
		...
		assert!(matches!(vec[1].value(),Value::Number(num) if matches!(num,Number::Float(f) if f==&1.2)));
	...
	fn complex() {
		...
		let Value::Array(inner) = vec[1].value() else { unreachable!() };
		assert_eq!(inner.len(), 2);
		assert!(matches!(inner[0].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&2)));
		assert_eq!(inner[0].raw(), "2");

		assert!(matches!(inner[1].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&3)));
		assert_eq!(inner[1].raw(), "3");

		let Value::Array(inner) = vec[2].value() else { unreachable!() };
		assert_eq!(inner.len(), 3);

		assert!(matches!(inner[0].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&4)));
		assert_eq!(inner[0].raw(), "4");

		let Value::Array(tmp) = inner[1].value() else { unreachable!() };
		assert_eq!(tmp.len(), 2);

		assert!(matches!(tmp[0].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&5)));
		assert_eq!(tmp[0].raw(), "5");

		assert!(matches!(tmp[1].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&6)));
		assert_eq!(tmp[1].raw(), "6");

		assert!(matches!(inner[2].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&7)));
		assert_eq!(inner[2].raw(), "7");

		assert!(matches!(vec[3].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&8)));
