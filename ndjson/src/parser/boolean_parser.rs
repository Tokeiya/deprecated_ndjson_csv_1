...
use crate::elements::{Value, BooleanValue};
use super::white_space::ws;
pub fn boolean<I: Stream<Token=char>>() -> impl Parser<I, Output=Value> {
	let t = chr::string::<I>("true").map(|_| {
		(true, "true")
	});

	let f = chr::string::<I>("false").map(|_| {
		(false, "false")
	});
	...
	(ws(), b, ws()).map(|(l, (v, t), r)| {
		Value::from(BooleanValue::new(v, format!("{}{}{}", l, t, r)))
	})
}
...
mod test {
	...
	fn bool() {
		...
		let (v, r) = parser.parse(input.as_str()).unwrap();
