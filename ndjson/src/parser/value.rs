...
mod test {
	...
	fn value() {
		...
		assert!(
			matches!(vec[1].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&2))
		);
		assert_eq!(vec[1].raw(), "2");

		assert!(
			matches!(vec[2].value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&3))
		);
		assert_eq!(vec[2].raw(), "3");

		let Value::Object(obj) = vec[3].value() else {
			unreachable!()
		};
		assert_eq!(obj.len(), 1);
