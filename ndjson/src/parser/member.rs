...
mod test {
	...
	fn member() {
		...
		assert!(
			matches!(act.value().value(),Value::Number(num) if matches!(num,Number::Integer(i) if i==&42))
		);
		...
		assert_eq!(act.name(), "foo");
		assert_eq!(act.value().raw(), "42");
		assert!(matches!(act.value().value(),Value::Number(num)
			if matches!(num,Number::Integer(i) if i==&42)))
	}
}
